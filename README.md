...
- 🎯 Entry points to execute your program using `python -m <ah_datahub_szapi>` or `$ ah_datahub_szapi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/vicky4u65/ah-datahub-szapi/branch/main/graph/badge.svg?token=ah-datahub-szapi_token_here)](https://codecov.io/gh/vicky4u65/ah-datahub-szapi)
[![CI](https://github.com/vicky4u65/ah-datahub-szapi/actions/workflows/main.yml/badge.svg)](https://github.com/vicky4u65/ah-datahub-szapi/actions/workflows/main.yml)

Awesome ah_datahub_szapi created by vicky4u65

## Install

from source
```bash
git clone https://github.com/vicky4u65/ah-datahub-szapi ah_datahub_szapi
cd ah_datahub_szapi
make install
```
...
```bash
pip install ah_datahub_szapi
```

## Executing

```bash
$ ah_datahub_szapi run --port 8080
```

or

```bash
python -m ah_datahub_szapi run --port 8080
```

or

```bash
$ uvicorn ah_datahub_szapi:app
```

## CLI

```bash
❯ ah_datahub_szapi --help
Usage: ah_datahub_szapi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ ah_datahub_szapi create-user --help
Usage: ah_datahub_szapi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
ah_datahub_szapi create-user admin admin --superuser
```
...
Run with `ah_datahub_szapi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `ah_datahub_szapi create-user`.
...
-----------------------------------------------------
ah_datahub_szapi/__init__.py              4      0   100%
ah_datahub_szapi/app.py                  16      1    94%
ah_datahub_szapi/cli.py                  21      0   100%
ah_datahub_szapi/config.py                5      0   100%
ah_datahub_szapi/db.py                   10      0   100%
ah_datahub_szapi/models/__init__.py       0      0   100%
ah_datahub_szapi/models/content.py       47      1    98%
ah_datahub_szapi/routes/__init__.py      11      0   100%
ah_datahub_szapi/routes/content.py       52     25    52%
ah_datahub_szapi/routes/security.py      15      1    93%
ah_datahub_szapi/routes/user.py          52     26    50%
ah_datahub_szapi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in ah_datahub_szapi/default.toml.
